...
const TradesPage = {
    render: async () => {
        ...
        const renderTradeList = (trades, currentUser) => {
            const ownedStickerMap = new Map(currentUser.ownedStickers.map(s => [s.sticker._id, s.quantity]));

            const canUserAfford = (requestedItems) => {
                for (const item of requestedItems) {
                    if (item.isCredit) {
                        if ((currentUser.credits || 0) < item.amount) return false;
                    } else if (item.sticker) {
                        const ownedQuantity = ownedStickerMap.get(item.sticker._id) || 0;
                        if (ownedQuantity < item.quantity) return false;
                    }
                }
                return true;
            };
            ...
            return publicTrades.map(trade => {
                const canAccept = canUserAfford(trade.requestedItems);
                const acceptButton = canAccept
                    ...
                    : `<button class="btn btn-sm btn-secondary" disabled title="You don't have the required items for this trade.">Cannot Accept</button>`;

                return `
                    <div class="card trade-offer-card mb-3 ${canAccept ? 'trade-available' : 'trade-unavailable'}">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <span>Offer by: <strong>${trade.proposerId.username}</strong></span>
                            ${canAccept ? '<span class="badge bg-success">Available</span>' : '<span class="badge bg-danger">Unavailable</span>'}
                        </div>
                        <div class="card-body">
                            <div class="row">
...
const renderStickerSelector = (id, stickers, isUserOwned) => {
    ...
    return `
        <div class="mb-3">
            ...
            ${searchBar}
            <div id="${id}-list" class="sticker-select-list" style="max-height: 220px; overflow-y: auto; border: 1px solid #ddd; border-radius: 4px; padding: 8px;">${options}</div>
        </div>`;
};
...
const getAcceptConfirmationMessage = (user, trade) => {
    const lastStickerNames = trade.requestedItems
        .filter(item => item.sticker && user.ownedStickers.some(s => (s.sticker._id === item.sticker._id || s.sticker === item.sticker) && s.quantity === 1))
        .map(item => item.sticker.name || 'Unknown Sticker');

    const alreadyOwnedNames = trade.proposerItems
        .filter(item => item.sticker && user.ownedStickers.some(s => s.sticker._id === item.sticker._id || s.sticker === item.sticker))
        .map(item => item.sticker.name || 'Unknown Sticker');
    ...
    const needsSpecialConfirm = lastStickerNames.length > 0 || alreadyOwnedNames.length > 0;
    if (needsSpecialConfirm) {
        confirmMsg += '\n';
    }
    confirmMsg += 'Are you sure you want to accept this trade? Make sure you have the requested items!';

    return { needsSpecialConfirm, confirmMsg };
};
...
const handleAcceptTrade = async (e) => {
    ...
    const { confirmMsg } = getAcceptConfirmationMessage(user, trade);
    const confirmed = await showConfirmModal(confirmMsg, 'Accept Trade', 'Cancel');
